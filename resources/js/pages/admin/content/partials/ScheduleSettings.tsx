...
export default function ScheduleSettings({ initialData }: { initialData: any }) {
    const { data, setData, post, processing, recentlySuccessful } = useForm({
        content: {
            title: initialData?.title || '',
            subtitle: initialData?.subtitle || '',
            button_label: initialData?.button_label || '',
            schedules: initialData?.schedules || [{ day: 'Mercredi', hours: '14h - 18h' }],
        },
    ...
    const updateSchedule = (index: number, field: 'day' | 'hours', value: string) => {
        const newSchedules = [...data.content.schedules];
        ...
        <form onSubmit={submit}>
            <Card className="border-sidebar-border/70 shadow-sm">
                <CardHeader>
                    ...
                    <CardDescription>
                        Configurez les jours d'ouverture et l'image d'illustration.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4 pt-4 border-t border-sidebar-border">
                        ...
                        <div className="space-y-3">
                            {data.content.schedules.map((schedule: any, index: number) => (
                            ...
                            {data.content.schedules.length === 0 && (
                                <p className="text-sm text-muted-foreground italic text-center py-4">Aucun horaire défini.</p>
                            )}
                        </div>
                    </div>
                </CardContent>
